...
Pole rescue operations will arrange points of light in the
sky to guide missing Elves back to base. Unfortunately, the
message is easy to miss: the points move slowly enough that
it takes hours to align them, but have so much momentum that
...
and record their position in the sky and their velocity, the
relative change in position per second (your puzzle input).
...
Each line represents one point. Positions are given as <X,
Y> pairs: X represents how far left (negative) or right (positive)
the point appears, while Y represents how far up (negative) or down (positive) the point appears.

At 0 seconds, each point has the position given. Each second,
each point's velocity is added to its position. So, a point
with velocity <1, -2> is moving to the right, but is moving
upward twice as quickly. If this point's initial position were <3, 9>, after 3 seconds, its position would become <6, 3>.
...
void getPointsFromString(PointsAry& points) {
...
TEST(TestRt, Day10_0) {
  PointsAry points;
  getPointsFromString(points);
  std::cout << "Got " << points.size() << " points." << std::endl;
  EXPECT_EQ(points.size(), 369);
  EXPECT_EQ(points[ 0 ]->startX, -30052);
  EXPECT_EQ(points[ 0 ]->startY, -9918);
  EXPECT_EQ(points[ 368 ]->startX, 20344);
  EXPECT_EQ(points[ 368 ]->startY, 20343);
  EXPECT_EQ(points.size(), 369);
  
  std::for_each(points.begin(), points.end(), [](std::shared_ptr<Point> p) {
    p->Print();
  });
}
...
TEST(TestRt, Day10) {
  ...
  const int minSeconds = getSecondsWithWhenMessageIsSmallest(points, secondsIncrement);
  ...
  for (int seconds = minSeconds; seconds < minSeconds + secondsIncrement;
    ...
    auto getMaxDim = [ &minX, &maxX, &minY, &maxY ](
    ...
    auto prickOff = [ &tbl, minX, minY, widthX, widthY ](
                      std::shared_ptr<Point> p1) {
