...
extern "C" {
    ...
    fn parse(input: *const i8) -> i64;
    // int64_t parse_do_dont(const char *input) {
    fn parse_do_dont(input: *const i8) -> i64;
}
...
mod tests {
    ...
    fn test_parse() {
        ...
        let c_string = CString::new(data_bytes).expect("CString::new failed");
    ...
    fn test_parse_do_dont() {
        ...
        // Replace "*" with "x"
        //let data_bytes = data_bytes.replace("^", "x").replace("do()", "^");
        //let data_bytes = data_bytes.replace("!", "x").replace("don't()", "!");
